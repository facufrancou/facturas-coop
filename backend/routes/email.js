...
function obtenerClientesListos() {
    ...
    for (const cliente of clientes) {
        ...
        const facturaCSV = facturasCSV.find(factura => String(parseInt(factura.suministro, 10)) === suministroCliente);
        if (!facturaCSV) {
            registrarClienteNoEnviado(cliente, 'Suministro no encontrado en facturas');
            continue;
        }
        const facturaPDF = archivosPDF.find(file => file.includes(facturaCSV.factura));
        if (!facturaPDF) {
...
function registrarMailEnviado(cliente, factura, email) {
    ...
    const logMessage = `${now.toISOString()} | Nombre: ${cliente.Nombre}, CUIT: ${cliente.cuit}, Email: ${email}, Factura: ${factura.factura}, Periodo: ${factura.periodo}\n`;
    ...
    } catch (e) { console.error('Error limpiando informes:', e); }
}
...
router.post('/enviar-individual', async (req, res) => {
    ...
    if (!Email) {
        registrarClienteNoEnviado({ Nombre, cuit, Codigo, Email }, 'Sin email registrado');
        return res.status(400).json({ message: 'El cliente no tiene email registrado.' });
    }

    const suministroCliente = parseInt(Codigo);
    const facturaCSV = facturasCSV.find(factura => parseInt(factura.suministro) === suministroCliente);

    if (!facturaCSV) {
        registrarClienteNoEnviado({ Nombre, cuit, Codigo, Email }, 'Suministro no encontrado en facturas');
        return res.status(404).json({ message: 'No se encontró la factura para el suministro.' });
    }

    const facturaPDF = fs.readdirSync(facturasDir).find(file => file.includes(facturaCSV.factura));

    if (!facturaPDF) {
        registrarClienteNoEnviado({ Nombre, cuit, Codigo, Email }, 'Archivo PDF no encontrado');
        return res.status(404).json({ message: 'No se encontró el archivo PDF de la factura.' });
    }
...
>>>>>>> a32f0ce5092b8e47fbe3a65c2459ee19e2b83ba4
        ...
        registrarClienteNoEnviado({ Nombre, cuit, Codigo, Email }, 'Error al enviar email');
...
=======
    ...
    for (let i = 0; i < clientes.length; i++) {
        ...
        try {
            const subject = 'Su Factura - Coop. Gral José de San Martín';
            const text = `Hola ${cliente.Nombre}, adjunto encontrarás la factura ${facturaCSV.factura} del período ${facturaCSV.periodo} correspondiente al suministro ${cliente.Codigo}.`;
...
router.get('/descargar-informe/:year/:month', (req, res) => {
    ...
    const informeFile = path.join(informesDir, `informe_mails_${year}_${month}.log`);
...
router.get('/listar-informes', (req, res) => {
    try {
        const files = fs.readdirSync(informesDir)
            .filter(f => f.startsWith('informe_mails_'));
        const informes = files.map(f => {
            const match = f.match(/informe_mails_(\d{4})_(\d{2})\.log/);
            if (match) {
                return { year: match[1], month: match[2] };
            }
            return null;
        }).filter(Boolean).sort((a, b) => b.year !== a.year ? b.year - a.year : b.month - a.month);
        res.json(informes);
    } catch (e) {
        res.json([]);
    }
